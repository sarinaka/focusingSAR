...
B0=10e6;                % Baseband bandwidth is plus/minus B0
w0=pi2*B0;
fc=300e6;                  % Carrier frequency
wc=pi2*fc;
Xc=3.e3;                 % Range distance to center of target area
X0=1000;                   % target area in range is within [Xc-X0,Xc+X0]
%
% Case 1:
...
alpha=w0/Tp;             % Chirp rate (THIS IS THE CHIRP SLOPE)
wcm=wc-alpha*Tp;         % Modified chirp carrier (START FREQUENCY)
%
dx=c/(4*B0);             % Range resolution
%
dt=pi/(2*alpha*Tp);      % Time domain sampling (gurad band plus minus
%dt = 1 / (2*B0);         % 50 per) or use dt=1/(2*B0) for a general
                         % radar signal
